<div class="js-grid-products-container row no-mar ajax-in-results">
    <div id="data-design-total" class="hidden"></div>
    @foreach ($designs as $design)
    <div class="col-lg-3 col-md-4 col-xs-6 boot-style">
        <div class="products-grid-tile">

            @if ($design->isShoppable())
                <a class="link-area" href="{{ route('view.shop.design', ['designIdentifier' => $design->identifier, 'category' => $categoryFilter]) }}"></a>
            @endif
...
<script type="text/javascript">
    var element = document.getElementById('shop-results-count');
    element.innerHTML = '{{ $designs->total() }}@if ($designs->total() > 1) Results @else Result @endif Found';
    var element = document.getElementById('data-design-total');
    element.innerHTML = '{{ $designs->total() }}';
</script>